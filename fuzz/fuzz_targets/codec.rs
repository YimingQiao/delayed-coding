#![no_main]
use delayed_coding::{
    decode_interleaved_into, encode_interleaved_into, Model, TableOptions, Workspace,
};
use libfuzzer_sys::fuzz_target;

fn exercise<const D: u32, const L: usize>(model: &Model, payload: &[u8], output_size: usize) {
    let mut output = vec![0; output_size];
    let _ = decode_interleaved_into::<D, L>(model, payload, &mut output);
    ...
    decode_interleaved_into::<D, L>(model, &storage[range], &mut restored).unwrap();
